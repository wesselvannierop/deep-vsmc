...
class Euler(BaseSolver):
    ...
    def _solve(self, ode_fn, t_0, z_0):
        ...
        final_t, final_z, final_diff, final_decreasing = tf.while_loop(
            stop, body, [t_0, z_0, tf.constant(float("inf")), tf.constant(True)]
        )
...
class NaiveSolver(RicattiSolver):
    ...
    def __init__(
        self, step_size=0.5, horizon=5.0, threshold=1e-3, name="RicattiSolver"
    ):
    ...
    @staticmethod
    def _routine_without_custom_grad(
        solver: BaseSolver, A: tf.Tensor, transport_matrix: tf.Tensor
    ):
        ode_fn = make_ode_fun(A, transport_matrix)
        res = solver.solve(ode_fn, 0.0, tf.zeros_like(A))

        @tf.function
        def grad(d_delta):
            d_delta = tf.clip_by_value(d_delta, -1.0, 1.0)
...
def schur_decomposition(tensor, max_iter, sort=False):
    ...
    Q_res, U_res, _ = tf.while_loop(
        cond,
        body,
        [
            tf.eye(tensor.shape[1], batch_shape=[tensor.shape[0]]),
            tensor,
            tf.constant(0),
        ],
    )
    ...
    if sort:
        ...
        return ordered_Q, tf.matmul(
            tf.matmul(ordered_Q, tensor, transpose_a=True), ordered_Q
        )
...
def matrix_sign(tensor, n_iter, threshold=1e-5):
    ...
    sqrt_svd_vals = tf.stop_gradient(
        tf.math.sqrt(tf.linalg.svd(tensor, compute_uv=False))
    )
    # no need for backprop as this *should* not impact the result

    max_ = tf.reduce_max(sqrt_svd_vals)
    non_null_vals = tf.where(tf.abs(sqrt_svd_vals) > 0.0, sqrt_svd_vals, max_)

    min_non_zero = tf.reshape(tf.reduce_min(non_null_vals, 1), [-1, 1, 1])
    shifted_tensor = tensor + min_non_zero * eye
    newton_schulz_flag = tf.reduce_all(
        _matrix_norm(eye - tf.matmul(tensor, tensor), 1) < 1.0
    )
    ...
    def newton(X):
        ...
        scaled_X = tf.where(
            mask, scaling_factor * X, tf.eye(X.shape[1], batch_shape=[X.shape[0]])
        )
    ...
    def body(X, error, i):
        ...
        temp = tf.where(
            mask,
            0.5 * (tf.matmul(new_X, new_X) + new_X),
            tf.eye(new_X.shape[1], batch_shape=[new_X.shape[0]]),
        )
        new_error = tf.reduce_max(
            tf.linalg.norm(tf.matmul(temp, temp) - temp, ord=1, axis=[1, 2])
        )

        return new_X, new_error, tf.add(i, 1)

    i0 = tf.constant(0)
    init_error = 2.0 * threshold

    sign, _final_error, final_i = tf.while_loop(
        cond,
        body,
        [
            shifted_tensor,
    ...
    @tf.function
    def grad(d_sign):
        d_sign_non_null = tf.reshape(
            tf.reduce_max(tf.abs(d_sign), [1, 2]) > 0, [-1, 1, 1]
        )
        (grad_val,) = tf.gradients(sign, [tensor], d_sign)
        grad_val_non_nan = tf.math.is_finite(grad_val)
        grad_val = tf.where(
            tf.logical_and(d_sign_non_null, grad_val_non_nan), grad_val, 0.0
        )
...
def _block_matrix(I, J, K, L):
    ...
    [K, L]]
    """
    res = tf.concat([tf.concat([I, J], -1), tf.concat([K, L], -1)], -2)
    return res
...
def qr(projector):
    ...
    def grad(dq):
        ...
        clipped_r_diag = tf.where(r_diag > 0.0, r_diag, clipped_r_diag)
        ...
        def _triangular_solve(x):
            ...
            return tf.linalg.matrix_transpose(
                tf.linalg.triangular_solve(clipped_r, t_x, lower=False, adjoint=False)
            )

        grad_val = tf.matmul(q, _triangular_solve(tril))
        return tf.clip_by_value(
            grad_val, -1.0, 1.0
        )  # anything outside results from a numerical instability.
...
def _solve_petkov(A, transport_matrix, n, n_iter):
    ...
    projector = 0.5 * (
        tf.eye(2 * n, batch_shape=[transport_matrix.shape[0]]) - sign_matrix
    )
    Q = qr(projector)
    ...
    delta = tf.linalg.solve(
        upper_left, tf.linalg.matrix_transpose(lower_left), adjoint=True
    )
    ode_fun = make_ode_fun(A, transport_matrix)

    ode_res = tf.reduce_mean(tf.abs(ode_fun(0.0, delta)))
    mask = tf.reshape(ode_res < 1e-1, [-1, 1, 1])

    delta = tf.where(mask, delta, 0.0)
...
class PetkovSolver(RicattiSolver):
    ...
    def _call(transport_matrix, w, n_iter):
        ...
        @tf.function
        def grad(dres):
            dres_non_null = tf.reduce_max(tf.abs(dres), [1, 2]) > 0.0
            dres = _make_admissible(dres)
            d_transport, dw = tf.gradients(res, [transport_matrix, w], dres)

            d_transport = tf.where(
                tf.reshape(dres_non_null, [-1, 1, 1]), d_transport, 0.0
            )
            dw = tf.where(tf.reshape(dres_non_null, [-1, 1]), dw, 0.0)

            return (
                tf.clip_by_value(d_transport, -1.0, 1.0),
                tf.clip_by_value(dw, -1.0, 1.0),
                None,
            )
