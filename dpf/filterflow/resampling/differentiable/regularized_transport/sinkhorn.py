...
from dpf.filterflow.resampling.differentiable.regularized_transport.utils import (
    cost,
    max_min,
    softmin,
)
...
@tf.function
def _simple_sinkhorn_loop(
    log_alpha, log_beta, cost_xy, cost_yx, epsilon, threshold, max_iter
):
    ...
    def apply_one(a_y, b_x):
        ...
        a_y_new = 0.5 * (a_y + at_y)
        b_x_new = 0.5 * (b_x + bt_x)
    ...
    total_iter, converged_a_y, converged_b_x, last_update_size = tf.while_loop(
        stop_condition,
        body,
        loop_vars=[n_iter, a_y_init, b_x_init, initial_update_size],
    )

    # We do a last extrapolation for the gradient - leverages fixed point + implicit function theorem
    a_y, b_x, _ = apply_one(
        tf.stop_gradient(converged_a_y), tf.stop_gradient(converged_b_x)
    )
...
def sinkhorn_loop(
    ...
    def apply_one(a_y, b_x, a_x, b_y, continue_, running_epsilon):
        ...
        at_y = tf.where(
            continue_reshaped,
            softmin(running_epsilon, cost_yx, log_alpha + b_x / running_epsilon_),
            a_y,
        )
        bt_x = tf.where(
            continue_reshaped,
            softmin(running_epsilon, cost_xy, log_beta + a_y / running_epsilon_),
            b_x,
        )

        at_x = tf.where(
            continue_reshaped,
            softmin(running_epsilon, cost_xx, log_alpha + a_x / running_epsilon_),
            a_x,
        )
        bt_y = tf.where(
            continue_reshaped,
            softmin(running_epsilon, cost_yy, log_beta + b_y / running_epsilon_),
            b_y,
        )
    ...
    def body(i, a_y, b_x, a_x, b_y, continue_, running_epsilon):
        new_a_y, new_b_x, new_a_x, new_b_y, local_continue = apply_one(
            a_y, b_x, a_x, b_y, continue_, running_epsilon
        )
    ...
    (
        ...
        tf.stop_gradient, (converged_a_y, converged_b_x, converged_a_x, converged_b_y)
    )
...
def sinkhorn_potentials(
    log_alpha, x, log_beta, y, epsilon, scaling, threshold, max_iter
):
    ...
    a_y, b_x, a_x, b_y, total_iter = sinkhorn_loop(
        log_alpha,
        log_beta,
        cost_xy,
        cost_yx,
