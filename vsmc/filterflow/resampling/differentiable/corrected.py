...
from vsmc.filterflow.resampling.base import ResamplerBase
from vsmc.filterflow.resampling.criterion import neff
from vsmc.filterflow.resampling.differentiable.biased import apply_transport_matrix
from vsmc.filterflow.resampling.differentiable.regularized_transport.plan import (
    transport,
)
from vsmc.filterflow.resampling.differentiable.ricatti.solver import PetkovSolver
from vsmc.filterflow.state import State
from vsmc.filterflow.utils import mean, std


@tf.function
def _fill_na(x):
    return tf.where(tf.math.is_finite(x), x, 1.0)
...
class CorrectedRegularizedTransform(ResamplerBase, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        super(CorrectedRegularizedTransform, self).__init__()
    ...
    def apply(self, state: State, flags: tf.Tensor, seed=None):
        ...
        res = apply_transport_matrix(
            state, transport_matrix + transport_correction, flags
        )
        return res
...
class PartiallyCorrectedRegularizedTransform(ResamplerBase, metaclass=abc.ABCMeta):
    ...
    # TODO: Document this really nicely
    def __init__(self, intermediate_resampler: ResamplerBase):
        """Constructor
        ...
        super(PartiallyCorrectedRegularizedTransform, self).__init__()
    ...
    def apply(self, state: State, flags: tf.Tensor, seed=None):
        ...
        alpha = tf.where(
            tf.reshape(ess > 3.0, [-1, 1, 1]),
            _make_alpha(weighted_std, transformed_std),
            1.0,
        )
        # Otherwise weighted std doesn't mean anything
        beta = (1.0 - alpha) * weighted_average
        particles = alpha * resampled_state.particles + beta

        particles = (
            tf.stop_gradient(particles - resampled_state.particles)
            + resampled_state.particles
        )

        new_state = resampled_state.evolve(particles=particles)
